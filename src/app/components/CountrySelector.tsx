...
const CountrySelector = ({ className }: CountrySelectorProps) => {
  ...
  const dispatch = useSetAtom(countryStateAtom);
  const setCountry = (code: string) => {
    const _country = countries.find((c) => c.code === code);
    if (_country) {
      dispatch({ type: "SET_COUNTRY", payload: _country });
    }
  };
  return (
    <Select onValueChange={(value) => setCountry(value)} defaultValue={country.code}>
